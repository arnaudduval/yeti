...
modeleIGA = IGAparametrization(filename='twoplatesDDcas1_U5_U102_C01')
...
ur = rigid_motion(*modeleIGA.get_inputs4rigid_motion(1, np.array([1.,0.]), np.array([-5.,2.5]), 0.))
ur = ur + rigid_motion(*modeleIGA.get_inputs4rigid_motion(2, np.array([1.,0.]), np.array([5.,2.5]), 0.))
...
# Check kernel for rigid motion 
g = (Ktot+(Ctot*Ktot.max()))@ur 
# print(Ktot.diagonal())
# print(np.mean(Ktot.diagonal()))
mean = np.mean(Ktot.diagonal())
# print(g)
...
if True:
    ...
    uref = np.loadtxt('/home/agagnaire/yeti/temp/UC0C1')
    # print(f'{u_iter[19] = }')
    # print(f'{u = }')
    # print(u-u_iter[19])
    ...
    # np.save('/home/agagnaire/yeti/temp/UCOC1', u)
    # np.savetxt('/home/agagnaire/yeti/temp/UC0C1', u)

    uC0 = np.loadtxt('/home/agagnaire/yeti/temp/UC0')

    print((np.linalg.norm(uC0)-np.linalg.norm(u))/(np.linalg.norm(u)))
