...
modeleIGA = IGAparametrization(filename='twoplatesDDcas1_U5')
...
additional_knots = {"patches":np.array([1]),"1":np.array([]),"2":np.array([0.333]),"3":np.array([])}
modeleIGA.refine(nb_ref,nb_deg,additional_knots)
additional_knots = {"patches":np.array([1]),"1":np.array([]),"2":np.array([0.666]),"3":np.array([])}
modeleIGA.refine(nb_ref,nb_deg,additional_knots)
...
if True:
    ...
    list_lgrge = np.where(np.isin(modeleIGA._ELT_TYPE,np.array([ 'U5'])))[0] + 1
