...
def test_romplatewithhole(tmp_path):
    # Read data and create IGAparametrization object
    script_dir = os.path.dirname(os.path.realpath(__file__))
    modelIGA = IGAparametrization(
    ...
    pp.generatevtu(*modelIGA.get_inputs4postprocVTU(
        'initialgeom',np.zeros((2,modelIGA._nb_cp),order='F'), nb_ref=nb_ref_visu, Flag=output, output_path=tmp_path))
    ...
    output = np.array([False, False, False])  # Output type: [disp, stress, VM]
    # pp.generatevtu(*modelIGA.get_inputs4postprocVTU(
    #     'modifiedgeom',np.zeros((2,modelIGA._nb_cp),order='F'), nb_ref=nb_ref_visu, Flag=output, output_path=tmp_path))
    ...
    for designvar_value in sample:
        ...
        # pp.generatevtu(*modelIGA.get_inputs4postprocVTU(
        #     'igasolution%02d'%i,SOL.T, nb_ref=nb_ref_visu, Flag=output, output_path=tmp_path))

        Urom = romsolution(designvar_value)
        SOL, u = rsol.reconstruction(**modelIGA.get_inputs4solution(Urom))
        # pp.generatevtu(*modelIGA.get_inputs4postprocVTU(
        #     'romsolution%02d'%i,SOL.T, nb_ref=nb_ref_visu, Flag=output, output_path=tmp_path))
        SOL, u = rsol.reconstruction(**modelIGA.get_inputs4solution(Urom-Uiga))
        # pp.generatevtu(*modelIGA.get_inputs4postprocVTU(
        #     'romerror%02d'%i,SOL.T, nb_ref=nb_ref_visu, Flag=np.array([True,False,False]), output_path=tmp_path))
