...
The shape of a Kirchhoff-Love shell roof is optimized versus its maximal displacement
...
n0 = optPB.compute_displacementAggreg(x0,pnorm=Pnorm)
...
def maxdisp(xD,gradD):
    ni = optPB.compute_displacementAggreg(xD,pnorm=Pnorm)
    if gradD.size>0:
        global i
        i += 1
        print('\n--')
        print('Iter %3i' % i)
        gradD[:] = optPB.compute_gradDisplacementAggreg_AN(xD,pnorm=Pnorm)/n0
        
        # postprocessing
        pp.generatevtu(*optPB._coarseParametrization.get_inputs4postprocVTU(
            'OPT5-coarse%0.2d'%i,optPB._coarseParametrization._COORDS-optPB._initialCOORDS,
            nb_ref=2*ref_plot,Flag=Output))
        optPB._coarseParametrization.generate_vtk4controlMeshVisu('OPT5-coarse%0.2d'%i,0)
        ...
        pp.generatevtu(*optPB._fineParametrization.get_inputs4postprocVTU(
            'OPT5-fine%0.2d'%i,  SOL.transpose(), nb_ref=1*ref_plot, Flag=Output))
    return ni/n0
...
x_ref = np.array([0.1472491,  0.53377687, 0.53385731, 0.14725017, 0.14735826, 0.24791125,
                  0.50500495, 0.50501984, 0.24804221, 0.14727647, 0.53389746, 0.50526637,
                  0.66379548, 0.66400121, 0.5049124,  0.53418678, 0.53375444, 0.50543068,
                  0.66376945, 0.66413801, 0.504866,   0.53431571, 0.1474783,  0.2477457,
                  0.50501584, 0.50456767, 0.24810994, 0.14724689, 0.1472503,  0.53384915,
