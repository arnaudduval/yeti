...
def test_vtu_output_8patchs(tmp_path):
    """
    Solve analysis and write VTU result file
    """
    script_dir = os.path.dirname(os.path.realpath(__file__))
    iga_model = IGAparametrization(
        filename=f'{script_dir}/inputs/BentPipe_8patchs')
    ...
    nb_ref[:, 2] = np.array([1, 0, 1])
    nb_ref[:, 3] = np.array([1, 0, 1])
    nb_ref[:, 4] = np.array([1, 0, 1])
    nb_ref[:, 5] = np.array([1, 0, 1])
    nb_ref[:, 6] = np.array([1, 0, 1])
    nb_ref[:, 7] = np.array([1, 0, 1])
...
if __name__ == '__main__':
    test_vtu_output_8patchs('results')
