...
if __name__ == '__main__':
    ...
    movable = np.array([[1, 1], [3, 2], [4, 1], [4, 2], [5, 2], [7, 1],
                        ...
                        [16, 1], [16, 0], [17, 0],
                        [19, 1], [21, 2], [22, 1], [22, 2], [23, 2], [25, 1],   # End of hull
    ...
    for case in cases:
        iga_model = IGAparametrization(filename='inputs_embeddedbeam/'+case['filename'])

        # Move control points
        random.seed(3)
        # Move hull CPs
        # TODO dimensions of hull(20, 3, 1) should be handled as variable
        for move in movable[:movable.shape[0]//2]:
            # CP at index move[0] is moved in move[1] direction
            match move[1]:
                case 0:
                    iga_model.coords[move[1], move[0]] = 20.*(0.1 + 0.8*random.random())
                case 1:
                    iga_model.coords[move[1], move[0]] = 3.*(0.1 + 0.8*random.random())
                case 2:
