...
subroutine mf_wq_interpolate_cp_3d(coefs, nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, &
    ...
    call PBiCGSTAB(solv, mat, nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, nnz_u, nnz_v, nnz_w, &
            ...
            data_W_u, data_W_v, data_W_w, U_u, U_v, U_w, Deigen, &
...
subroutine mf_wq_steady_heat_3d(coefs, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, &
                            nnz_u, nnz_v, nnz_w, indi_u, indj_u, indi_v, indj_v, indi_w, indj_w, &
                            data_B_u, data_B_v, data_B_w, data_W_u, data_W_v, data_W_w, &
                            b, nbIterPCG, threshold, methodPCG, x, RelRes)
    ...
    double precision, dimension(:, :), allocatable :: U_u, U_v, U_w, temp
    double precision, dimension(:, :, :), allocatable :: newcoefs
    double precision, dimension(:), allocatable :: D_u, D_v, D_w, I_u, I_v, I_w
    double precision :: k_um, k_vm, k_wm, kmeanvector(3), kappa
    ...
    if (methodPCG.eq.'WP') then 
        ...
        if ((methodPCG.eq.'TDS').or.(methodPCG.eq.'TDC')) then 
            ...
            ! Compute mean
            call compute_mean_3d(nc_u, nc_v, nc_w, coefs(1, 1, :), k_um) 
            call compute_mean_3d(nc_u, nc_v, nc_w, coefs(2, 2, :), k_vm) 
            call compute_mean_3d(nc_u, nc_v, nc_w, coefs(3, 3, :), k_wm) 
        ...
        if ((methodPCG.eq.'C').or.(methodPCG.eq.'JMC')) then
            ...
            call compute_steady_condition_number(nc_total, coefs, kmeanvector, kappa)
            print*, 'Method:', methodPCG, 'Condition number: ', kappa
        
        else if (methodPCG.eq.'TDC') then
            allocate(newcoefs(3, 3, nc_total), temp(3, nc_total))
            temp = 0.d0
            call kronvec3d(nc_w, Mcoef_w, nc_v, Mcoef_v, nc_u, Kcoef_u, temp(1, :), 1.d0)
            call kronvec3d(nc_w, Mcoef_w, nc_v, Kcoef_v, nc_u, Mcoef_u, temp(2, :), 1.d0)
            call kronvec3d(nc_w, Kcoef_w, nc_v, Mcoef_v, nc_u, Mcoef_u, temp(3, :), 1.d0)
            do i = 1, 3
                do j = 1, 3
                    newcoefs(i, j, :) = (coefs(i, j, :)/sqrt(temp(i, :)))/sqrt(temp(j, :))
                end do
            end do
            call compute_steady_condition_number(nc_total, newcoefs, (/1.d0, 1.d0, 1.d0/), kappa)
            print*, 'Method:', methodPCG, 'Condition number: ', kappa
        end if
...
subroutine mf_iga_steady_heat_3d(coefs, nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, nnz_u, nnz_v, nnz_w, &
                                ...
                                nbIterPCG, threshold, methodPCG, x, RelRes)
    !! (Preconditioned) Conjugate gradient algorithm to solver linear heat problems 
    !! It solves Ann xn = bn, where Ann is Knn (steady heat problem) and bn = Fn - And xd
    !! bn is compute beforehand (In python)
    !! IN CSR FORMAT
...
subroutine mf_wq_transient_nonlinear_3d(nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, &
    ...
    double precision, parameter :: thresholdPCG = 1.d-12, thresholdNL=1.d-12
    ...
    do i = 2, sizeF
        ...
        do j = 1, nbIterNL
            ...
            call interp_temperature(nr_u_t, nc_u_t, nr_v_t, nc_v_t, nr_w_t, nc_w_t, nnz_u_t, nnz_v_t, nnz_w_t, &
                                        indi_u_t, indj_u_t, indi_v_t, indj_v_t, indi_w_t, indj_w_t, &
                                        data_B_u_t, data_B_v_t, data_B_w_t, TTn1, TTinterp)

            ! Interpolate capacity and conductivity at each quadrature point 
            call interp_isotropic_prop(nbpts, table_cap, nc_total, TTinterp, CC)
            call interp_isotropic_prop(nbpts, table_cond, nc_total, TTinterp, KK)
            
            ! Compute coefficients to compute tangent matrix
            call eval_isotropic_coefs(nc_total, KK, CC, invJJ, detJJ, Kcoefs, Ccoefs)
