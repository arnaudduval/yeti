...
subroutine wq_get_forceSurf_3d(coefs, nc_total, nr_u, nc_u, nr_v, nc_v, nnz_u, nnz_v, &
    ...
    ! -------------
    double precision :: result_temp
    dimension :: result_temp(nr_u*nr_v)
    integer :: i
    ...
    do i = 1, d
        do j = 1, d
            call mf_wq_get_ku_3d(coefs((i-1)*d+1:i*d, (j-1)*d+1:j*d, :), nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, & 
                            ...
                            data_BT_u, data_BT_v, data_BT_w, indi_u, indj_u, indi_v, indj_v, indi_w, indj_w, &
                            data_W_u, data_W_v, data_W_w, array_input((j-1)*nnz+1:j*nnz), array_temp)

            init = (j + (i-1)*d - 1)*nnz + 1
            fin = (j + (i-1)*d)*nnz
...
subroutine wq_mf_static_3d(coefs, nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, &
                            ...
                            U_u, U_v, U_w, Deigen, nbIter, b, x, RelRes)
    ...
    dimension :: x(nr_total*d), RelRes(nbIter+1)
...
subroutine solver_plasticity(nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, &
                        nnz_u, nnz_v, nnz_w, indi_u, indj_u, indi_v, indj_v, indi_w, indj_w, &
                        data_B_u, data_B_v, data_B_w, data_W_u, data_W_v, data_W_w, &
                        table, JJ, E, nu, sigma_Y, nbsteps, Fext, disp)
    ...
    dimension ::    data_B_u(nnz_u, 2), data_W_u(nnz_u, 4), &
                    ...
                    data_B_w(nnz_w, 2), data_W_w(nnz_w, 4)
    ! Physics
    integer, intent(in) :: nbsteps, table
    dimension :: table(d, d, 2)
    double precision, intent(in) :: E, nu, sigma_Y
    double precision, intent(in) :: JJ, Fext
    dimension :: JJ(d, d, nc_total), Fext(nr_total*d, nbsteps)
    character(len = 10) :: Method = 'FDC'
    
    double precision, intent(out) :: disp
    dimension :: disp(nr_total*d, nbsteps+1)
    ...
    double precision :: ep_n1, ep_n0
    dimension :: ep_n1(ddl, nc_total), ep_n0(ddl, nc_total)
    double precision :: disp_temp, delta_disp, e_n1, sigma_n1, dSdE, coef_fint, coef_S, Fint
    dimension ::    disp_temp(nr_total*d), delta_disp(nr_total*d), e_n1(ddl, nc_total), &
                    sigma_n1(ddl, nc_total), dSdE(ddl, ddl, nc_total), coef_fint(d*d, nc_total), & 
                    coef_s(d*d, d*d, nc_total), Fint(nr_total*d)
    ...
    do i = 2, nbsteps+1
        ...
        do j = 1, nbIterRaphson
            ...
            ! Update F
            Fint = Fext(:, i) - Fint
            error = maxval(abs(Fint))/maxval(abs(Fext(:, i)))
            ...
            if (error.le.1e-6) then 
                ...
                call  wq_mf_static_3d(coef_S, nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, &
                                    nnz_u, nnz_v, nnz_w, indi_u, indj_u, indi_v, indj_v, indi_w, indj_w, &
                                    data_B_u, data_B_v, data_B_w, data_W_u, data_W_v, data_W_w, &
                                    U_u, U_v, U_w, Deigen, nbIterSolver, Fint, delta_disp, dummy)
