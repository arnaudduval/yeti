...
subroutine mf_wq_elasticity_3d_py(nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, &
    ...
    kwargs(1, :) = mat%lambda; kwargs(2, :) = mat%mu
    call setupArgs(mat, nc_total, kwargs)

    allocate(I_u(nr_u), I_v(nr_v), I_w(nr_w))
    I_u = 1.d0; I_v = 1.d0; I_w = 1.d0
    if (methodPCG.eq.'JMC') call compute_coef_mean_3d(mat, nc_u, nc_v, nc_w)
...
subroutine mf_wq_plasticity_3d(nr_total, nc_total, nr_u, nc_u, nr_v, nc_v, nr_w, nc_w, &
    ...
    do i = 2, sizeF
        ...
        do j = 1, nbIterNL ! Newton-Raphson solver
            ...
            ! Compute Deigen
            call compute_coef_mean_3d(mat, nc_u, nc_v, nc_w)
            do k = 1, dimen
                mean = mat%mean(i, :)
