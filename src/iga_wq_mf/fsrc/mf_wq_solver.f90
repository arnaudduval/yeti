...
subroutine mf_wq_get_cu_3d( nb_cols_total, capacity_coefs, &
    ...
    ! Initialize
    allocate(array_temp_1(nb_cols_u*nb_cols_v*nb_cols_w))
    array_temp_1 = 0.d0
...
subroutine mf_wq_get_ku_3d_coo( nb_rows_total, nb_cols_total, cond_coefs, &
    ...
    dimension ::    indi_W_u_csr(nb_rows_u+1), &
                    indi_W_v_csr(nb_rows_v+1), &
                    indi_W_w_csr(nb_rows_w+1)
    integer, allocatable, dimension(:) :: indj_W_u_csr, indj_W_v_csr, indj_W_w_csr
    double precision, allocatable, dimension(:) :: data_W_u_csr, data_W_v_csr, data_W_w_csr
    ...
    allocate(data_W_u_csr(size_data_u), &
            data_W_v_csr(size_data_v), &
            data_W_w_csr(size_data_w))
    allocate(indj_W_u_csr(size_data_u), &
            indj_W_v_csr(size_data_v), &
            indj_W_w_csr(size_data_w))
    call coo2csr(nb_rows_u, size_data_u, data_W00_u, indi_u, indj_u, data_W_u_csr, &
                    indj_W_u_csr, indi_W_u_csr)
    call coo2csr(nb_rows_v, size_data_v, data_W00_v, indi_v, indj_v, data_W_v_csr, &
                    indj_W_v_csr, indi_W_v_csr)
    call coo2csr(nb_rows_w, size_data_w, data_W00_w, indi_w, indj_w, data_W_w_csr, &
                    indj_W_w_csr, indi_W_w_csr)
    deallocate(data_W_u_csr, data_W_v_csr, data_W_w_csr)
    deallocate(indj_W_u_csr, indj_W_v_csr, indj_W_w_csr)
    ! =======================================================
...
end subroutine mf_wq_get_ku_3d_coo

! ----------------------------------------
! Conjugate gradient
! ----------------------------------------
subroutine wq_mf_cg_3d(nb_cols_total, coefs, table_block, &
                        nb_rows_u, nb_cols_u, &
                        nb_rows_v, nb_cols_v, &
                        nb_rows_w, nb_cols_w, &
                        size_data_u, size_data_v, size_data_w, &
    ...
    if (Method.eq.'WP') then 
        ...
        if ((Method.eq.'TDS').or.(Method.eq.'TD')) then 
            ...
            ! NEW METHOD
            ! -------------------------------------------- 
            call jacobien_mean_3d(nb_cols_u, nb_cols_v, nb_cols_w, Jacob, Lu, Lv, Lw)
        end if
        ...
        call eigen_decomposition(nb_rows_u, nb_cols_u, Mcoef_u, Kcoef_u, size_data_u, &
                                indi_W_u_csr, indj_u, data_B0_u, data_W00_u, data_B1_u, &
        ...
        call eigen_decomposition(nb_rows_v, nb_cols_v, Mcoef_v, Kcoef_v, size_data_v, &
                                indi_W_v_csr, indj_v, data_B0_v, data_W00_v, data_B1_v, &
        ...
        call eigen_decomposition(nb_rows_w, nb_cols_w, Mcoef_w, Kcoef_w, size_data_w, &
                                indi_W_w_csr, indj_w, data_B0_w, data_W00_w, data_B1_w, &
        ...
        if ((Method.eq.'TDS').or.(Method.eq.'JMS')) then
            ...
            allocate(matrixdiag(nb_rows_u*nb_rows_v*nb_rows_w))
...
subroutine wq_mf_bicgstab_3d(nb_cols_total, coefs, table_block, &
                            nb_rows_u, nb_cols_u, &
                            nb_rows_v, nb_cols_v, &
                            nb_rows_w, nb_cols_w, &
                            size_data_u, size_data_v, size_data_w, &
    ...
    double precision :: r, rhat, p, Ap, s, As
    dimension :: r(size_dof), rhat(size_dof), p(size_dof), Ap(size_dof), As(size_dof), s(size_dof)
    double precision :: v1exp, v2exp
    dimension :: v1exp(nb_rows_u*nb_rows_v*nb_rows_w), &
                    v2exp(nb_rows_u*nb_rows_v*nb_rows_w)
    integer :: k
    ...
    dimension :: ptilde(size_dof), Aptilde(size_dof), Astilde(size_dof), stilde(size_dof)
