...
class problem():
	...
	def L2projectionCtrlpts(self, u_atqp, table=None, prop=None):
		...
		if self.part.dim == 2: volForce = geophy.get_forcevol_2d(*inpts)
		if self.part.dim == 3: volForce = geophy.get_forcevol_3d(*inpts)
		u_interp = self._solveL2projection(np.atleast_2d(volForce), table=table, prop=prop)
		return u_interp
...
class heatproblem(problem):
	...
	def solveFourierTransientProblemHighOrderImplicit(self, Tinout, Fext_list, time_list, isLumped=False):
		
		assert False, 'TODO: THE ALGORITHM IS NOT CONVERGING'

		def computeVelocity(problem:heatproblem, force, displacement, velocity, isLumped=False):
			temperature = problem.interpolate_temperature(displacement)
			args = {'temperature':temperature, 'position':problem.part.qpPhy}
			dof = problem.boundary.thdof

			vel2return = np.copy(velocity); vel2return[dof] = 0.0
			residual = force - problem.compute_mfConductivity(displacement, args=args) - problem.compute_mfCapacity(vel2return, args=args, isLumped=isLumped) 

			if isLumped: tmp = residual/problem.compute_mfCapacity(np.ones(problem.part.nbctrlpts_total), args=args, isLumped=isLumped)
			else: tmp = problem._solveL2projection(residual, prop=problem.heatmaterial.capacity(args)*problem.heatmaterial.density(args),
													table=problem.boundary.thDirichletTable)
			vel2return[dof] = np.copy(tmp[dof])

			return vel2return

		def gauss_step(problem:heatproblem, d_n1, v_n1, Fext_n1, dt, isLumped=False):
			
			dod = self.boundary.thdod
			dof = problem.boundary.thdof
			dj_n1 = np.copy(d_n1)
			k0 = computeVelocity(problem, Fext_n1, dj_n1, v_n1, isLumped=isLumped)
			d1_guess = dj_n1 + (1/2 - np.sqrt(3)/6) * dt * k0; d1_guess[dod] = np.copy(d_n1[dod])
			d2_guess = dj_n1 + (1/2 + np.sqrt(3)/6) * dt * k0; d2_guess[dod] = np.copy(d_n1[dod])
			k1_guess = computeVelocity(problem, Fext_n1, d1_guess, v_n1, isLumped=isLumped)
			k2_guess = computeVelocity(problem, Fext_n1, d2_guess, v_n1, isLumped=isLumped)
			...
			def error(k1, k2):
				e1 = k1 - computeVelocity(problem, Fext_n1, d_n1 + (a11 * k1 + a12 * k2) * dt, v_n1, isLumped=isLumped)
				e2 = k2 - computeVelocity(problem, Fext_n1, d_n1 + (a21 * k1 + a22 * k2) * dt, v_n1, isLumped=isLumped)
				return np.concatenate((e1, e2))

			j = 0			
			while j < self._itersNL:
				er = error(k1_guess, k2_guess)
				resNLj = np.sqrt(np.dot(er, er))
				if j == 0: resNL0 = resNLj
				print('NonLinear error: %.5e' %np.linalg.norm(er))
				if resNLj <= max([self._safeguard, self._thresNL*resNL0]): break

				temperature = problem.interpolate_temperature(dj_n1)
				args = {'temperature':temperature, 'position':problem.part.qpPhy}
				
				res1 = k0 - a12*dt*problem.compute_mfConductivity(k2_guess, args=args); res1[dod] = 0.0
				tmp1, _ = self._solveLinearizedTransientProblem(res1, a11*dt, args=args, isLumped=isLumped)

				res2 = k0 - a21*dt*problem.compute_mfConductivity(k1_guess, args=args); res2[dod] = 0.0
				tmp2, _ = self._solveLinearizedTransientProblem(res2, a22*dt, args=args, isLumped=isLumped)
				
				k1_guess[dof] = np.copy(tmp1[dof])
				k2_guess[dof] = np.copy(tmp2[dof])
				
				dj_n1[dof] = d_n1[dof] + dt/2 * (k1_guess[dof] + k2_guess[dof])
				j += 1				

			d_n1[dof] = np.copy(dj_n1[dof])
			return
		...
		for i in range(1, nsteps):
			...
			Vj_n1 = np.zeros(nbctrlpts_total)
			dj_n1 = np.copy(Tinout[:, i-1])
			Vj_n1[dod] = 1.0/dt*(Tinout[dod, i] - dj_n1[dod])
			dj_n1[dod] = Tinout[dod, i]

			gauss_step(self, dj_n1, Vj_n1, Fext_n1, dt, isLumped=isLumped)
			Tinout[:, i] = np.copy(dj_n1)
