...
from lib.base_functions import create_knotvector, iga_find_basis_weights_opt
...
class Timoshenko():
    ...
    def verify_mechaprop(self):
        ...
        if any([item is None for item in item_list]): 
            self.compute_mechaprop()
        return
...
def compute_sub_force_vector(J, DB, W, prop):
    ...
    coefs = prop * J * W 
    # coefs = np.atleast_2d(coefs).reshape(-1, 1)
    F = DB[0] @ coefs
    return F
