...
def exactTemperature(qpPhy, nlcase=NLCASE):
	...
	if nlcase==1: u = 0.001*(-5*x + 6*y + 45)*(5*x + 6*y - 45)*x*(x-6)*np.sin(np.pi*t)
	if nlcase==2: raise Warning('Not done yet')
	return u
...
def simulate(degree, cuts, quadArgs, problemArgs={}, isRandom=False, nlcase=NLCASE):
	# Create model 
	if nlcase==1:
		geoArgs = {'name': 'tp', 'degree': degree*np.ones(3, dtype=int), 
					'nb_refinementByDirection': cuts*np.ones(3, dtype=int)}
	if nlcase==2:
		raise Warning('Not done yet')
...
for quadrule, quadtype, plotpars in zip(['iga'], ['leg'], [normalPlot]):
	...
	for i, degree in enumerate(degree_list):
		...
		if quadrule == 'iga': 
			...
			slope = round(slope, 1)
			annotation.slope_marker((nbels[3], error_list[3]), slope, 
							poly_kwargs={'facecolor': (0.73, 0.8, 1)}, ax=ax)			
		else: 
			ax.loglog(nbels, error_list, color=color, marker=plotpars['marker'], markerfacecolor='w',
					markersize=plotpars['markersize'], linestyle=plotpars['linestyle'])
			
		fig.savefig(folder + 'SPTNonLinearConvergenceL2' + '.pdf')
...
ax.set_ylim(top=1e2, bottom=1e-11)
ax.legend()
fig.tight_layout()
fig.savefig(folder + 'SPTNonLinearConvergenceL2' + '.pdf')
