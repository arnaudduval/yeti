...
qp_wq, B, W, indi, indj = wq_find_basis_weights_fortran(degree, knotvector, method=2)[1:]
indi -= 1; indj -= 1
DB, DW = [], []
...
# Solve using IGA
disp_iga, strain, stress = solve_IGA_plasticity_1D(properties_iga, DB=basis_cgg, W=weight_cgg, Fext=Fext, dof=dof)
strain_cp   = interpolate_IGA_CP_1D(basis_cgg, weight_cgg, strain)
stress_cp 	= interpolate_IGA_CP_1D(basis_cgg, weight_cgg, stress)
plot_results(degree, knotvector, JJ, disp_iga, strain_cp, 
                stress_cp, folder=folder, method='IGA')

# Solve using WQ
disp_wq, strain, stress = solve_WQ_plasticity_1D(properties_wq, DB=DB, DW=DW, Fext=Fext, dof=dof)
strain_cp   = interpolate_WQ_CP_1D(DB, DW, strain)
stress_cp   = interpolate_WQ_CP_1D(DB, DW, stress)
plot_results(degree, knotvector, JJ, disp_wq, strain_cp, 
