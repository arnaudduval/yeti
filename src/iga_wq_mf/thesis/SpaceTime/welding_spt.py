...
# Select folder
full_path = os.path.realpath(__file__)
folder = os.path.dirname(full_path) + '/results/heat_transfer/'
if not os.path.isdir(folder): os.mkdir(folder)
...
def run(folder=None):
	...
	g = VtkGroup(folder)
	for i in range(33):
		g.addFile(filepath = folder + "out2_"+str(i)+".vts", sim_time = i)
	g.save()
...
def simulate(degree, cuts, quadArgs, cuts_time=None):
	...
	# Block boundaries
	tmptable = np.ones((3, 2)); tmptable[-1, 1] = 0
	sptnbctrlpts = np.array([*modelPhy.nbctrlpts[:modelPhy.dim], time_spt.nbctrlpts_total])
	boundary_spt = boundaryCondition(sptnbctrlpts)
	boundary_spt.add_DirichletConstTemperature(table=tmptable)
...
for i in range(0, 66, 2): 
	vtk2png(folder, cmap='coolwarm', filename='out_'+str(i), title='Temperature', clim=[-15, 15], n_colors=21)
	filename = folder + 'out_' + str(i) + 'temp.png'
	images.append(imageio.imread(filename))
images += images[-2::-1] 
imageio.mimsave(folder + 'heat_transfer.gif', images)
