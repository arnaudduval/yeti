...
NBSTEPS = 65
TIMELIST = np.linspace(0., 2., NBSTEPS) 
geonameList = ['vb']
ITERMETHODS = ['C', 'JMC']
RUNSIMU = False
...
if RUNSIMU:
	...
	# problem, Tinout, _ = simulation_heattransfer(geoname='vb')
	# for k, i in enumerate(range(0, np.size(Tinout, axis=1), 2)):
	...
	for i in range(33): 
		vtk2png(folder, filename='out_'+str(i), title='Temperature', clim=[0, 10], n_colors=21, camera_position='yx', position_y=0.25)

	images = []
	for i in range(33):
		filename = folder + 'out_' + str(i) + 'temp.png'
		images.append(imageio.imread(filename))
	images += images[-2::-1] 
	imageio.mimsave(folder + 'temperature.gif', images)
...
for geoname in geonameList:
	fig, ax = plt.subplots()
	for [i, precond], color in zip(enumerate(ITERMETHODS), colorlist):
		filename = FOLDER2DATA + 'ResidualHt_' + geoname + '_' + precond + '.dat'  
		AllresLin = np.loadtxt(filename)

		if precond == "C": labelmethod = 'Standard Fast Diag.'
		elif precond == "JMC": labelmethod = 'Our contribution'
