...
nbelList = [2**i for i in np.arange(2, 6)]
...
for degree in [1, 2]:
	...
	fig.savefig(FOLDER2SAVE+'tweights'+'W0_'+str(degree)+'.png')
	...
	for i in range(weightedQuad.nbctrlpts):
		ax.plot(quadPos, np.ravel(weightsmatrix[i, :]))
	fig.savefig(FOLDER2SAVE+'tweights'+'W1_'+str(degree)+'.png')
	...
	fig.savefig(FOLDER2SAVE+'tbasis'+'B0_'+str(degree)+'.png')
	...
	for i in range(weightedQuad.nbctrlpts):
		ax.plot(quadPos, np.ravel(basismatrix[i, :]))
	fig.savefig(FOLDER2SAVE+'tbasis'+'B1_'+str(degree)+'.png')
...
for ax, varName in zip(np.ravel(axs), ['I00', 'I01', 'I10', 'I11']):
	for degree in range(2, 5):
		...
		ax.loglog(nbelList, errorList, '-o', label=label)
...
fig.tight_layout()
fig.savefig(FOLDER2SAVE + 'terrorweights' + '.png')