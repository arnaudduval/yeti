...
from pysrc.lib.lib_base import (createUniformKnotvector_Rmultiplicity, cropImage)
from pysrc.lib.lib_quadrules import *
from pysrc.lib.lib_geomdl import Geomdl
from pysrc.lib.lib_part import part

def plot2DGeo(modelpart:part, sampleSize=101):
	"Plots a 2D geometry "
	...
	ctrlpts = modelpart.ctrlpts
	evalpts = modelpart.interpolateMeshgridField(sampleSize=sampleSize*np.ones(3,dtype=int))[0]

	# Get basis using fortran
	basis, indices = [], []
	for i in range(modelpart.dim):
		dersb, indi, indj = evalDersBasisCSRFortran(modelpart.degree[i], modelpart.knotvector[i], np.unique(modelpart.knotvector[i]))
		basis.append(dersb); indices.append(indi); indices.append(indj)

	# Get position and determinant 
	nbknots = [len(np.unique(modelpart.knotvector[i])) for i in range(modelpart.dim)]
	inpts = [*nbknots, *indices, *basis, modelpart.ctrlpts]
	...
	ax.grid(None)
	plot_mesh(ctrlpts, modelpart.nbctrlpts, ax)
...
if FIGCASE == 0: # B-spline curve
	...
	def case22(folder, extension, is2D=False):

		# Set filename
		filename = folder + 'BivariateFunctions' 
	...
	case2(FOLDER2SAVE, EXTENSION)
	case22(FOLDER2SAVE, EXTENSION)
	...
	# Set filename
	filename = FOLDER2SAVE + 'BsplineSurface' + EXTENSION
	...
	def case6(folder, extension, isDers=0):
		...
		ax2 = ax1.twinx()
		ax2.plot(quadRule.quadPtsPos, weightsval, 'ko')
		plotVerticalLine(quadRule.quadPtsPos, weightsval, ax2)
		ax2.set_ylim(ylim2)
		ax2.grid(None)
	...
	case6(FOLDER2SAVE, EXTENSION)

elif FIGCASE == 7: # 2D Geometries
