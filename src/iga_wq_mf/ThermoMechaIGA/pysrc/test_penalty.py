...
DD, UU = eigen_decomposition(indi, indj, data, t_robin=[0, 0])

# Get some matrices
B0, B1, W00, _, _, W11 = array2csr(indi, indj, B, W)
M = W00 @ B0.T; M = M.todense()
K = W11 @ B1.T; K = K.todense()

def fast_diag_1d(DD, UU, dof, u_in):

    # Find number of rows
    nrows = len(DD)

    # Create vector
    vector1 = np.zeros(nrows)
    vector1[dof] = u_in

    # Define output
    DDt = DD + 1
    vector2 = UU @ ((UU.T @ vector1)/DDt) 
    u_out = vector2[dof]

    return u_out

def compute_Ku_1d(nrows, K, u_in, dof):
...
def conjugate_gradient_1D(DD, UU, K, F, ug, dod, dof, nbIterations=20, epsilon=1e-10):   
    "Solves L K L un = L F using preconditionned conjugated gradient "
    
    # Create x
    xlen = len(dod) + len(dof)
    un = np.zeros(len(dof))
    x = np.zeros(xlen)
    x[dod] = ug
    Kx = K @ x; Kx = np.squeeze(np.asarray(Kx))
    FKx = F - Kx
    ...
    # ------------------
    r = FKx[dof] 
    z = fast_diag_1d(DD, UU, dof, r)
    p = z
    rsold = np.dot(r, z)

    for k in range(nbIterations):
        Ap = compute_Ku_1d(xlen, K, p, dof)
        alpha = rsold/np.dot(p, Ap)
        un = un + alpha*p
        r = r - alpha*Ap

        # if residue < epsilon: break
        x[dof] = un
        z = fast_diag_1d(DD, UU, dof, r)
        rsnew = np.dot(r, z)
        p = z + rsnew/rsold * p
...
T3 = conjugate_gradient_1D(DD, UU, K, F, [0, 0], dod1D, dof1D)
print(T3)
