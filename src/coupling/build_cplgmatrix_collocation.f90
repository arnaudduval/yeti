subroutine cplg_matrix_collocation(xi_master, xi_slave, xi_inter, i_master, i_slave, &
            ...
            &  Cdata1, Crow1, Ccol1, Cdata2, Crow2, Ccol2, x_phys_m)
...
double precision, intent(out):: x_phys_m
dimension x_phys_m(3)
...
double precision, dimension(3) :: xi_slave_inv, xi_inter_inv, xi_master_inv, x_phys, x_phys_s
...
call updateElementNumber(xi_inter)
call evalnurbs(xi_inter, R_lgrge, dRdxi_lgrge)
...
do icp = 1, nnode_patch
    ...
    do idim = 1, MCRD
        ...
        x_phys(idim) = x_phys(idim) + &
            &   R_lgrge(icp)*COORDS_elem(idim,icp)        
    enddo
enddo
...
! if ((x_phys(1)**2 + x_phys(2)**2) <= 1**2) then
    ...
    call point_inversion_surface(x_phys, 5, COORDS3D, nb_cp,   &
        &   .false., xi_master_inv, info)

    call updateElementNumber(xi_master_inv)
    ...
    call evalnurbs(xi_master_inv, R_master, dRdxi_master)

    ! write(*,*) 'xi_master', xi_master_inv
    ...
    do icp = 1, nnode_patch
        ...
        do idim = 1, MCRD
            ...
            x_phys_m(idim) = x_phys_m(idim) + &
                &   R_master(icp)*COORDS_elem(idim,icp)
        enddo
    enddo

    write(*,*) 'x_phys_m', x_phys_m
    ! write(*,*) dxdxi_master
    ...
    do icp = 1, nnode_patch
        ...
        do idim = 1, MCRD
            ...
            x_phys_s(idim) = x_phys_s(idim) + &
                &   R_slave(icp)*COORDS_elem(idim,icp)
        enddo
    enddo

    write(*,*) 'x_phys_s', x_phys_s
