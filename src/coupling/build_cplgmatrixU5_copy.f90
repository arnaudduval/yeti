...
subroutine cplg_matrixU5(nb_data, &
    ...
    do iPatch = 1, nb_patch
        ...
        if(ELT_TYPE_patch .eq. 'U5') then
            ...
            ! write(*,*) 'ATTENTION VALEUR DE NB_GPS FORCE A 9'
            ! nb_gps = 18
                ...
                do igps = 1, nb_gps
                    ...
                    if(idom .eq. 1) then
                        ...
                        elseif (derivCoupling .eq. 1) then 
                            ...
                            & dxdxi_master(:(dim_patch+1),:(dim_patch+1), igps), weightGP(igps), &
                            & NNODE(iPatch), nnode_patch, dim_patch+1, dim_patch+1, masterFace, CMAT(:,:n))
                        endif
                    else
                        ...
                        if (derivCoupling .eq. 0) then
                            ...
                            & NNODE(iPatch), nnode_patch, MCRD, CMAT(:,:n))
                        elseif (derivCoupling .eq. 1) then
                            call cplingddispU5(R_lgrge(:, igps), dRdxi_slave(:, :, igps), & 
                            & dxdxi_slave(:(dim_patch+1),:(dim_patch+1), igps), weightGP(igps), &
                            & NNODE(iPatch), nnode_patch, dim_patch+1, dim_patch+1, slaveFace, CMAT(:,:n))
                        endif
                    endif
                    i = 0
    ...
    do j = 1, NNODE_l
        do i = 1, NNODE_d
            do l = 1, dim_patchd
                do m = 1, MCRD
                    do k = 1, MCRD
                        CMAT(k,count) = CMAT(k,count) + Rl(j)*dRdxid(i,l)*dxidx(l,m)*norm_vect(m)*detJ
                    enddo
                enddo
            enddo
            count = count+1
