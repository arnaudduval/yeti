...
!! with Yeti. if not, see <https://www.gnu.org/licenses/>
     ...
     1     NBINT, COORDS, COORDSall, TENSOR, MATERIAL_PROPERTIES, 
     2     DENSITY, nb_load, indDLoad, load_target_nbelem, JDLType,
     3     ADLMAG, load_additionalInfos, nb_load_additionalInfos, 
     4     n_dist_elem, nb_n_dist, RHS, AMATRX)
     ...
     1                                AJmat(:, 2), AJmat(:, 3), DetJac)
                              case(2)                ! Face 2
                                  call VectNormNorm(VectNorm, 
     1                                AJmat(:, 3), AJmat(:, 2), DetJac)
                              case(3)                ! Face 3
                                  call VectNormNorm(VectNorm, 
     1                                AJmat(:, 3), AJmat(:, 1), DetJac)
                              case(4)                ! Face 4
                                  call VectNormNorm(VectNorm, 
     1                                AJmat(:, 1), AJmat(:, 3), DetJac)
                              case(5)                ! Face 5
                                  call VectNormNorm(VectNorm, 
     1                                AJmat(:, 1),AJmat(:, 2), DetJac)
                              case(6)                ! Face 6
                                  call VectNormNorm(VectNorm, 
     1                                AJmat(:, 2), AJmat(:, 1), DetJac)
                          end select
