...
subroutine sys_linmat_lindef_static_omp(Kdata,Krow,Kcol,F,  &
    ...
    integer :: loccount, idx, nb_data_elem
    ...
    do NumPatch = 1,nb_patch
        ...
        nb_data_elem = MCRD*MCRD*(nnode_patch*(nnode_patch-1))/2        &
            &          + nnode_patch * (MCRD * (MCRD+1))/2

        ! write(*,*) 'Hello', nb_elem_patch(NumPatch), omp_get_thread_num()
        !! Loop on elements
        !$omp parallel default(none) shared(nb_elem_patch, NumPatch, activeElement, JELEM, nnode_patch, mcrd) &
        ...
        do num_elem = 1,nb_elem_patch(NumPatch)
            ! write(*,*) num_elem
            !!JELEM = JELEM + 1
            ! write(*,*) "*****Element ", JELEM+num_elem, nb_elem_patch(NumPatch), omp_get_thread_num(), "********"
            ! write(*,*) COORDS3D
            if (activeElement(JELEM+num_elem)==1) then
                call extractNurbsElementInfos(num_elem)

                write(*,'(I4,I4,I4,6F4.1)') num_elem, current_elem, omp_get_thread_num()!!, Ukv_elem
                ...
                elseif (ELT_TYPE_patch == 'U1') then
                    !! Solid element
                    call UELMAT_byCP(NDOFEL,MCRD,nnode_patch,JELEM+num_elem, &
                ...
                elseif (ELT_TYPE_patch == 'U99') then
                    ...
                    call UELMAT_HO_byCP(NDOFEL,MCRD,nnode_patch,JELEM+num_elem, &
                        &   NBINT(NumPatch),COORDS_elem(:,:nnode_patch),    &
                ...
                elseif (ELT_TYPE_patch == 'U98') then
                    ...
                    call UELMAT_HO_byCP_1stG(NDOFEL,MCRD,nnode_patch,JELEM+num_elem, &
                        &   NBINT(NumPatch),COORDS_elem(:,:nnode_patch),    &
                        &   TENSOR_patch,MAT_patch,n_mat_props(NumPatch), RHO(NumPatch),nb_load,   &
                        &   indDLoad,load_target_nbelem,JDLType,ADLMAG, &
                        &   load_additionalInfos,SIZE(load_additionalInfos),&
                ...
                elseif (ELT_TYPE_patch == 'U3') then
                    !! Shell element
                    call UELMAT3_byCP(NDOFEL,MCRD,nnode_patch,JELEM+num_elem,        &
            	...
            	elseif (ELT_TYPE_patch == 'U10') then
                    ...
                    call UELMAT10(NDOFEL,MCRD,nnode_patch,nnode_map,nb_cp,      &
                        &   JELEM+num_elem,NBINT(NumPatch),COORDS_elem(:,:nnode_patch),  &
                ...
                elseif (ELT_TYPE_patch == 'U30') then
                    ...
                    call UELMAT30(NDOFEL,MCRD,nnode_patch,nnode_map,nb_cp,      &
                        &   JELEM+num_elem,NBINT(NumPatch),COORDS_elem,COORDS3D,         &
                ...
                idx = count + (num_elem-1) * nb_data_elem
                ! write(*,*) nnode_patch
                    ...
                    do cpi = 1,cpj-1
                        ...
                        do ll = 1,MCRD
                            do kk = 1,MCRD
                                ...
                                Krow(idx + loccount) = dofi + kk - 1
                                Kcol(idx + loccount) = dofj + ll - 1
        ...
        count = count + nb_elem_patch(Numpatch) * nb_data_elem
